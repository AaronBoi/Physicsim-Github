...
bool running = true;
bool reset = true;
bool periodic_border = false;
bool walls_with_mouse = false;
bool walls_top_bottom = false; 
float cylinder_radius = width / 12;
bool gravity = false;
...
void init()
{
    memset(n_arr, 1, sizeof(n_arr));
    memset(wall_arr, 0, sizeof(wall_arr));
    memset(rho_arr, 1, sizeof(rho_arr));
    ...
    for (int x = 0; x < width; x++) {
        for (int y = 0; y < height; y++) {
            ...
            for (int i = 0; i < dims; i++){
                ...
                const float eDotU = e_arr[i][0] * ux * (1.0 + 0.01 * distribution(generator)) + e_arr[i][0] * uy;
